...
interface OrderItem {
  ...
  productSize: string | null;
  quantity: number;
  returnedQuantity?: number; // Added returnedQuantity field
  supplierPrice: number;
  variantColor: string | null;
  variantId: string | null;
  variantSize: string | null;
...
export default function OrderDetailsModal({
  ...
  const getAvailableQuantity = (item: OrderItem) => {
    const returnedQuantity = item.returnedQuantity || 0;
    return item.quantity - returnedQuantity;
  };

  // Helper function to check if item is fully returned
  const isFullyReturned = (item: OrderItem) => {
                    ...
                    <CardContent>
                      <div className="space-y-4">
                        {supplier.orderItems.map((item, itemIndex) => {
                          ...
                          const returnedQuantity = item.returnedQuantity || 0;
                          const isFullyReturnedItem = isFullyReturned(item);
                          ...
                          return (
                            ...
                            >
                              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                                ...
                                {isFullyReturnedItem ? (
                                  <div className="flex items-center justify-center min-w-[200px] text-red-600 font-medium">
                                    All items returned
                                  </div>
                                ) : (
                                  <div className="flex flex-col gap-3 min-w-[200px]">
